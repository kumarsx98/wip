...
describe('samlp', function () {
  ...
  describe('configured signature signatureNamespacePrefix', function(){
    ...
    describe('configured with assertion transform', function() {
      let body, $, signedAssertion, samlResponse;
      const CUSTOM_ATTRIBUTE = `<saml:Attribute xmlns:x500="urn:oasis:names:tc:SAML:2.0:profiles:attribute:X500"
                             x500:Encoding="LDAP"
                             NameFormat="urn:oasis:names:tc:SAML:2.0:attrname-format:uri"
                             Name="urn:oid:2.5.4.42"
                             FriendlyName="givenName">
                               <saml:AttributeValue xsi:type="xsd:string">Custom Foo</saml:AttributeValue>
                           </saml:Attribute>`;

      before(function (done) {
        server.options = {
          transformAssertion: function (assertion) {
            const attributes = assertion.getElementsByTagNameNS('urn:oasis:names:tc:SAML:2.0:assertion', 'AttributeStatement')[0];
            attributes.appendChild(new xmldom.DOMParser().parseFromString(CUSTOM_ATTRIBUTE))
          }
        };
        request.get({
          jar: request.jar(),
      ...
      it('should return custom attribute extension inside the assertion', function() {
        ...
        expect(attributes.length).to.equal(6);

        const transformedAttribute = attributes[5];
        expect(transformedAttribute.getAttribute('NameFormat')).to.equal('urn:oasis:names:tc:SAML:2.0:attrname-format:uri');
        expect(transformedAttribute.getAttribute('Name')).to.equal('urn:oid:2.5.4.42');
        expect(transformedAttribute.getAttribute('FriendlyName')).to.equal('givenName');
        expect(transformedAttribute.childNodes.length).to.equal(1);
        expect(transformedAttribute.childNodes[0].textContent).to.equal('Custom Foo');
