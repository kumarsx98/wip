...
describe('saml 2.0', function () {
  ...
  function saml20TestSuite(options) {
    ...
    describe('#' + createAssertion, function () {
      ...
      it('should transform assertion by adding a custom attribute with complex type', function () {
        const CUSTOM_ATTRIBUTE = `<saml:Attribute xmlns:x500="urn:oasis:names:tc:SAML:2.0:profiles:attribute:X500"
                                     x500:Encoding="LDAP"
                                     NameFormat="urn:oasis:names:tc:SAML:2.0:attrname-format:uri"
                                     Name="urn:oid:2.5.4.42"
                                     FriendlyName="givenName">
                                       <saml:AttributeValue xsi:type="xsd:string">Foo</saml:AttributeValue>
                                   </saml:Attribute>`;
        ...
        const signedAssertion = saml[createAssertion](options);
        const attributes = utils.getAttributes(signedAssertion)
        assert.equal(attributes.length, 2);

        const transformedAttribute = attributes[1];
        assert.equal(transformedAttribute.getAttribute('NameFormat'), 'urn:oasis:names:tc:SAML:2.0:attrname-format:uri');
        assert.equal(transformedAttribute.getAttribute('Name'), 'urn:oid:2.5.4.42');
        assert.equal(transformedAttribute.getAttribute('FriendlyName'), 'givenName');
        assert.equal(transformedAttribute.childNodes.length, 1);
        assert.equal(transformedAttribute.childNodes[0].textContent, 'Foo');
      });
