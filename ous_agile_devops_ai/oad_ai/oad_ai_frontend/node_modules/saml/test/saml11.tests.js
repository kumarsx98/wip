  ...
  function saml11TestSuite(options) {
    ...
    describe('#' + createAssertion, function () {
      ...
      it('should transform assertion by adding a custom attribute with complex type', function () {
        const CUSTOM_ATTRIBUTE = `<saml:Attribute 
        ...
        const options = {
          ...
          transformAssertion: function(assertion) {
            const attributes = assertion.getElementsByTagNameNS('urn:oasis:names:tc:SAML:1.0:assertion', 'AttributeStatement')[0];
            attributes.appendChild(new xmldom.DOMParser().parseFromString(CUSTOM_ATTRIBUTE))
          }
        };

        const signedAssertion = saml11[createAssertion](options);
        ...
        assert.equal(transformedAttribute.getAttribute('AttributeNamespace'), 'urn:mace:shibboleth:1.0:attributeNamespace:uri');
        assert.equal(transformedAttribute.getAttribute('AttributeName'), 'urn:mace:dir:attribute-def:givenName');
